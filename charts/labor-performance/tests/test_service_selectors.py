...
Run: python3 charts/labor-performance/tests/test_service_selectors.py
"""
...
def render(extra_args: list[str]) -> list[dict]:
    out = subprocess.run(
        ["helm", "template", "labor-performance", str(CHART_DIR), *extra_args],
...
def main() -> int:
    ...
    if "labor-performance" not in services:
        failures.append("the OLTP Service was not rendered")
    elif selector_of(services["labor-performance"]).get("app.kubernetes.io/component") != "api":
        failures.append("the OLTP Service selector must pin component=api")

    if "labor-performance-frontend" not in services:
        failures.append("the frontend Service was not rendered with frontend.enabled=true")
    elif selector_of(services["labor-performance-frontend"]).get("app.kubernetes.io/component") != "frontend":
    ...
    fe = services.get("labor-performance-frontend")

---
id: 0008-standard-metrics-convention
slug: /adr/0008-standard-metrics-convention
title: 0008. Standard metrics convention across the fleet
sidebar_label: 0008. Standard metrics convention
description: ADR 0008 — the fleet-wide OTel Tier 1/Tier 2 metrics convention (otelchi HTTP RED instrumentation, a shared business-metric naming shape) and this service's gap closure to full compliance.
---

# 0008. Standard metrics convention across the fleet
...
**This service specifically:** by the time this ADR reaches
`labor-performance`, the earlier fleet-wide gap in `telemetry.Setup` itself
had already been closed here — `internal/adapters/outbound/telemetry/telemetry.go`
already installs the MeterProvider/TracerProvider and calls
`runtime.Start(runtime.WithMeterProvider(...))`, and `cmd/labor/main.go`
already calls `telemetry.Setup` at startup. What remained missing, and what
this PR brings into compliance, was Tier 1 item 2 — `otelchi` HTTP RED
instrumentation was entirely absent from
`internal/adapters/inbound/http/server.go`'s `NewRouter` (only
`RequestID`, `RequestLogger`, `Recoverer`, and CORS middleware were wired)
— and Tier 2: this service had zero business metric of any kind. This PR
adds both, closing `labor-performance` out from the two open items the
fleet-wide audit above found for it.
...
`labor-performance`'s own gap closure (this PR): `NewRouter` now wires
`otelchi.Middleware` and `otelchimetric.NewServerRequestDuration` in the
mandated position (after `RequestID`, before `RequestLogger`), and
`DefineStandard` now records `labor_performance.standards.defined` —
attribute `outcome=accepted|rejected` — via a new
`ports.StandardMetrics` port implemented by
`internal/adapters/outbound/telemetry/metrics.go`, mirroring
`inventory-storage`'s `ReservationMetrics` shape exactly (nil-safe, wired
once in `cmd/labor/main.go`'s composition root, called from the use case
